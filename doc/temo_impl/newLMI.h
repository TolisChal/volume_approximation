...
public:
    ...
    void get_full_matrix(MT const& mat, MT &res) {}
...
public:
    ...
    void get_full_matrix(MT const& mat, MT &res) 
    {
        ...
        tripletList.clear();
            ...
            {
                for (typename MT::InnerIterator it(mat, k); it; ++it)
                {
                    //Rcpp::Rcout << " i=" << i_.index() << " value=" << i_.value() << std::endl;
                    //row_col = get_position_in_matrix(it.row(), _m);
                    tripletList.push_back(T(it.row(), it.col(), it.value())); // get the lower triangular
                    if (it.row() != it.col()) // get the upper triangular
                    {
...
class LMI {
    ...
    void evaluateWithoutA0(const VT& x, MT& res, bool complete_mat = false) {
        if (!lmi_evaluator.evaluateWithoutA0(x, res, complete_mat)) {
            ...
            if (complete_mat) {
                MT ret;
                lmi_evaluator.get_full_matrix(res, ret);
                res = ret;
            }
