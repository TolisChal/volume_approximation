...
public:
    ...
    Spectrahedron(const LMI<NT, MT, VT>& _lmi) : lmi(_lmi) {
        d = _lmi.dimension();
        grad.setZero(d);
    }

    void initialize_lmi_at_c(VT &c) {
        MT lmi_c(lmi.sizeOfMatrices(), lmi.sizeOfMatrices());
        lmi.evaluateWithoutA0(c, lmi_c, true);

        #if defined(SPARSE_PROBLEM)
            Alu.analyzePattern(lmi_c);
            Alu.factorize(lmi_c);
        #elif defined(DENSE_PROBLEM)
            std::cout<<"lmi_c = \n"<<lmi_c<<"\n"<<std::endl;
            Alu.compute(lmi_c);
    ...
    NT positiveIntersection(VT const & a, VT const & b, VT const & c, NT const& temperature, PrecomputedValues2& precomputedValues) 
    {
