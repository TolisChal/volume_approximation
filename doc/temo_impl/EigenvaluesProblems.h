...
#include "DenseProductMatrix_eff.h"
...
//                max = eivals(i).real();
    ...
    */
    ...
    NT minPosGeneralizedEigenvalue(MT const & A, MT const & B, MT const& C, CVT& eigenvector, 
                                   NT const& temperature, decomposition &Alu) {

        NT lambdaMinPositive = std::numeric_limits<NT>::max(), lambdaMinPositive2 = std::numeric_limits<NT>::max();
        ...
        std::cout<<"\n[1]lamdaminPositive = "<<lambdaMinPositive<<std::endl;


        DenseProductMatrixEff<NT, decomposition> _M(&A, &B, &C, temperature, &Alu);
        ...
        //const NT tol_ = 1e-04;
        //std::cout<<"tol_ = "<<tol_<<std::endl;
        ARNonSymStdEig<NT, DenseProductMatrixEff<NT, decomposition> >dprob2((2*A.cols()), 1, &_M, &DenseProductMatrixEff<NT, decomposition>::MultMv, 
        ...
        // retrieve eigenvalue of the original system
        lambdaMinPositive2 = 1/dprob2.EigenvalueReal(0);
        std::cout<<"[2]lamdaminPositive = "<<lambdaMinPositive2<<std::endl;

        VT eigenvector2(2*A.rows());
        if (dprob2.EigenvectorsFound()) {
            //retrieve corresponding eigenvector
            for (int i=0 ;i<(2*A.rows()) ; i++)
                eigenvector2(i) = dprob2.EigenvectorReal(0, i);
        }

        std::cout<<"[1]eigenvector = "<<eigenvector.transpose()<<std::endl;
        std::cout<<"[2]eigenvector2 = "<<eigenvector2.transpose()<<std::endl;
        //std::cout<<"[2.5]eigenvector2 = "<<(eigenvector2.block(A.rows(),0,A.rows(),1)).transpose()<<std::endl;
        //std::cout<<"[3]ratio = ";
        //for (int i=0 ;i<A.rows() ; i++) {
        //        std::cout<<" "<<eigenvector2(i) / eigenvector2(i+A.rows());
        //}
        //std::cout<<"\n";
        
        //std::cout<<"[4]dot "<<eigenvector2.block(0,0,A.rows(),1).dot(eigenvector.block(A.rows(),0,A.rows(),1))<<std::endl;
...
//        std::cout << lambdaMinPositive << " " << eigenvector.transpose() << "\n";fflush(stdout);
    ...
    template <typename decomposition>
    NT minPosQuadraticEigenvalue(MT const & A, MT const &B, MT const &C, VT &eigenvector,
                                 NT const& temperature, decomposition &Alu) 
    {
        ...
        CVT eivector;
        NT lambdaMinPositive = minPosGeneralizedEigenvalue(A, B, C, eivector, temperature, Alu);
