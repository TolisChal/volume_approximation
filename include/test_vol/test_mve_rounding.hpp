...
#include "preprocess/mve_computation.hpp"
...
template <
        ...
        typename NT
>
std::pair< std::pair< std::pair<MT, VT>, std::pair<MT, VT> >, NT > test_mve_rounding(Polytope &P, 
                                                ...
                                                MT &N, VT &N_shift)
{
    ...
    while (true)
    {
        ...
        T = T * L;
        round_val += std::log(L.transpose().determinant());
...
template <
        ...
        typename NT
>
std::pair< std::pair<MT, VT>, NT > test_mve_rounding(Polytope &P, std::pair<Point, NT> InnerBall)
{
    unsigned int d = P.dimension();
    MT N = MT::Identity(d,d);
    VT shift = VT::Zero(d);
    std::pair< std::pair< std::pair<MT, VT>, std::pair<MT, VT> >, NT > result = test_mve_rounding(P, InnerBall, N, shift);
