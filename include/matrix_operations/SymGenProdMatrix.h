...
#ifndef VOLESTI_SYMGENPRODMATRIX_H
#define VOLESTI_SYMGENPRODMATRIX_H
...
public:
    ...
    typedef Eigen::LDLT<MT, Eigen::Lower> Decomposition;
#else
    ...
    SymGenProdMatrix(MT const *A, MT const *B) : A(A), B(B) {
        Blu.compute(*B);
        _rows = A->rows();
        _cols = B->cols();
    }
